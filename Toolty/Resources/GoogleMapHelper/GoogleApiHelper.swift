...
/*
 Example of api url
 Near By  :https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=(lat),(long)&radius=5000&keyword=hospital&key=(yourkey)
 */
...
class GoogleApi : NSObject {
    ...
    func getConvertedInputFor(_ usedFor:UsedFor,input:GInput) -> String {
        ...
        case .reverseGeo:
            ...
            return ""
        case .nearBy:
            //location=(yourlatitude),(yourlongitude)&radius=500&keyword=airport&key=(yourkey)
            var finalParameters = ""
            if let lat = input.destinationCoordinate?.latitude, let lng = input.destinationCoordinate?.longitude {
                let lat = String(format:"%.14f", lat)
                let long = String(format:"%.14f", lng)
