...
//  Copyright © 2021 Multipz Technology. All rights reserved.
...
class JTValidationToast: UIView {
    ...
    class func show(message msg: String, viewController: UIViewController? = nil, type: ToastType = ToastType.error, presentType : ToastPosition = .top, animationStlye : AnimationStlye = .slide ,completion: (()->())? = nil) {
      ...
      //  toast.lblMessage.font = UIFont.ToolytFontWith(.openSansSemibold, size: 15.widthRatio)
        toast.clipsToBounds = true
        toast.viewMessage.backgroundColor = type == .success ? UIColor.green : UIColor.red
