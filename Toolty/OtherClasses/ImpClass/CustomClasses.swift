...
class JPTextField: UITextField {
    ...
    override func awakeFromNib() {
        ...
        tintColor = UIColor.hexStringToUIColor(hexStr: "0F4C71", alpha: 1.0)
        //textColor = UIColor.hexStringToUIColor(hexStr: "0F4C71", alpha: 1.0)
        if let place = placeholder {
            let font = UIFont.ToolytFontWith(.openSansRegular, size: 16.widthRatio)
            let color = UIColor.hexStringToUIColor(hexStr: "0F4C71", alpha: 0.5)
            setAttributedPlaceHolder(text: place, font: font, color: color, spacing: 0)
        }
        self.font = UIFont.ToolytFontWith(.openSansSemibold, size: 16.widthRatio)
    }
    
    func addAttributePlaceHolder(isAsterikRequired: Bool) {
        if let place = placeholder{
            let font = UIFont.ToolytFontWith(.openSansRegular, size: 15.widthRatio)
...
class KPRoundTextField: UITextField {
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        self.layer.masksToBounds = true
        tintColor = #colorLiteral(red: 0, green: 0.2, blue: 0.2, alpha: 1)
    }
}
...
class KPCustomImageView: UIImageView {
...
class KPSingleRoundCornerView: UIView{
    ...
    override func awakeFromNib() {
        ...
        mask.path = path.cgPath
        mask.lineWidth = 2
        mask.strokeColor = isTopLeft ? AvatrecColor.red.cgColor : AvatrecColor.gray.cgColor
