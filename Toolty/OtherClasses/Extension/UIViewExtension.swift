...
extension UIView {
    ...
    func applyGradientEffects(_ colours: [UIColor], gradientPoint: VBGradientPoint, removeFirstLayer: Bool = true) {
        layoutIfNeeded()
        if let subLayers = layer.sublayers, removeFirstLayer {
...
class BottomCornerRadiusView: UIView {
    ...
    @IBInspectable var cornerRadious: CGFloat = 24.0
...
enum VBGradientPoint {
    ...
    func draw() -> GradientType {
        ...
        case .topBottom:
            return (startPoint: CGPoint(x: 0.2, y: 0), endPoint: CGPoint(x: 0.2, y: 1))
        case .bottomTop:
